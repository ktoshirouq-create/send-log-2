...
const App = {
    ...
    // NEW V57 Feature: Form Validation for Save Button
    validateForm: () => {
    ...
    renderUI: () => {
        ...
        App.updateUISelections(); 
        App.validateForm(); // Validate button state instantly
    ...
    updateUISelections: () => {
        ...
        App.validateForm(); // Always double check the save button when soft painting
    },
    ...
    renderJournal: () => {
        ...
        let htmlOut = visibleSessions.map(session => {
            ...
            if (sends.length > 0) {
                ...
                if (sessionType.includes('Bouldering') && sConf.colors && sConf.colors.length > 0) {
            ...
            if (domDisc !== 'mixed') {
                if(children[0] && children[0].Type) {
                    const idx = AppConfig.disciplines.indexOf(children[0].Type);
                    if(idx > -1) domLabel = AppConfig.discLabels[idx];
                }
            }
            ...
            const childrenHtml = children.map(l => {
                ...
                if (l.Type === 'Indoor Bouldering') {
                    ...
                    if (idx > -1 && AppConfig.grades.bouldsIn.colors[idx]) inlineColor = `color: ${AppConfig.grades.bouldsIn.colors[idx]} !important;`;
                }
                ...
                if (l.Style && AppConfig.styles[l.Style]) metaHtml += `<div class="log-meta-item">STYLE<div class="log-meta-val">${AppConfig.styles[l.Style]}</div></div>`;
    ...
    renderDashboardLogs: () => {
        ...
        // OPTION A: Fully Interactive Dashboard Logs
        document.getElementById('logList').innerHTML = displayLogs.length === 0 ? '<div style="text-align:center; padding:20px; color:var(--text-muted);">No logs found.</div>' : displayLogs.map(l => {
            ...
            return `
            <div class="log-card" style="border-color: rgba(255,255,255,0.05); background: rgba(0,0,0,0.2);">
                <div class="log-summary" onclick="App.haptic(); const p = this.parentElement; const isExp = p.classList.contains('expanded'); document.querySelectorAll('#logList .log-card').forEach(c => c.classList.remove('expanded')); if(!isExp) p.classList.add('expanded');">
                    <div class="log-date" style="width: 50px;">${formattedDate}</div>
                    <div class="log-info" style="padding-left:0;">
                        <div class="log-name">${displayRoute}${syncWarning}</div>
                    </div>
                    <div class="log-grade ${isF ? 'fl' : (isFail ? 'fail' : 'rp')}" style="${inlineColor}">${badge}${finalDisplayGrade}</div>
    ...
    logClimb: () => {
        ...
        if (State.discipline.includes('Outdoor') && (!outName || !outCrag)) return; // Failsafe
        ...
        setTimeout(() => {
            ...
            setTimeout(() => { 
                ...
                btn.disabled = false; 
                App.validateForm(); // Re-validate just in case they switched to Outdoor while it was saving
            }, 2000);
        }, 400); 
    }
};
App.init();if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js').catch(err => console.log('SW Registration failed:', err)));
}
