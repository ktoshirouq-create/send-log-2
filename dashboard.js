...
// Expose these so the HTML onclicks can find them
...
// V34: Dashboard Master Object for Logbook State
const Dashboard = {
    sortCol: 'Date',
    sortAsc: false,
    searchQuery: '',
    ...
    sortLogbook: (col) => {
        if (Dashboard.sortCol === col) {
            Dashboard.sortAsc = !Dashboard.sortAsc; // Toggle direction
        } else {
            Dashboard.sortCol = col;
            Dashboard.sortAsc = false; // Default to descending for new column
        }
    ...
    renderLogbook: () => {
        ...
        displayData.sort((a, b) => {
            let valA, valB;
            if (Dashboard.sortCol === 'Date') { valA = new Date(getV(a, 'Date')).getTime(); valB = new Date(getV(b, 'Date')).getTime(); }
            else if (Dashboard.sortCol === 'Name') { valA = getV(a, 'Name').toLowerCase(); valB = getV(b, 'Name').toLowerCase(); }
            else if (Dashboard.sortCol === 'Grade') { valA = Number(getV(a, 'Score')) || 0; valB = Number(getV(b, 'Score')) || 0; }
            else if (Dashboard.sortCol === 'Style') { valA = getV(a, 'Style').toLowerCase(); valB = getV(b, 'Style').toLowerCase(); }
        ...
        tbody.innerHTML = displayData.map(l => {
            ...
            // Try to find the associated session for fatigue data
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    allSessionsMaster = allSessions; // Export to global for Logbook
    ...
    function renderDashboard() {
        ...
        const limit = [...currentFilteredLogs].filter(l => (getV(l, 'Effort')||"").includes('Limit') || (getV(l, 'GradeFeel')||"").includes('Hard')).sort((a,b)=>(Number(getV(b, 'Score'))||0)-(Number(getV(a, 'Score'))||0)).slice(0,5);
        ...
        AppConfig.steepness.forEach(st => {
            const logsForSt = currentFilteredLogs.filter(l => (getV(l, 'Angle')||"").includes(st) && getV(l, 'Score'));
        ...
        // V34: Render Master Logbook at the end of updating filters
        Dashboard.renderLogbook();
    }
